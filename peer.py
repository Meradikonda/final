...
SERVER_CONN_BYE = 'SERVER CONN BYE'
PEER_QUIT = 'PEER QUIT'
PEER_QUIT_ACK = 'PEER QUIT ACK'
            ...
            # get events on central server
                ...
                try:
                    ...
                    if data:
                        ...
                        elif data[0] == SERVER_CONN_BYE:
                            server.close()
                            print(f'[ SERVER ] {data[1]}')

                        elif data[0] == PEER_QUIT_ACK:
                            print(f'[ SERVER ] {data[1]}')
                            server.close()
                            our_server.close()
                            sys.exit()
                ...
                if isCommand(msg, '/quit'):
                    server.send(pickle.dumps([PEER_QUIT, [ourHost, ourPort]]))
                    
                    # for conn in active_conn:
                        # conn.send(pickle.dumps([PEER_QUIT]))                    
                    ...
                    if peer is not server and peer is not our_server and peer is not sys.stdin:
                        try:
                            ...
                            if pdata:
                                ...
                                elif pdata[0] == PEER_QUIT:
                                    peer.send(pickle.dumps([PEER_QUIT_ACK, 'connection closed']))
                                    peer.close()
                                    print(f'[ PEER ] peer {peer} is exiting...')
