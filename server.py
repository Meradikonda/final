    ...
    while True:
        try:
            ...
            for sock in sockets_rd: 
                # sock.send(pickle.dumps([SERVER_CONN_BYE, 'Goodbye :)']))              
                sock.close()
                sockets_rd.remove(sock)            
            sys.exit()
        ...
        for event in events_rd:
            if event == s:
                ...
                print(f'[ INFO ] PEER {paddr} CONNECTED TO SERVER')
                ...
                for sock_to_rcv in sockets_rd:
                    if sock_to_rcv != s and sock_to_rcv is event:
                        ...
                        if data:
                            ...
                            elif data[0] == PEER_QUIT:
                                print(f'[ PEERS ] Peer {sock_to_rcv} is exciting')
                                sock_to_rcv.send(pickle.dumps([PEER_QUIT_ACK, 'connection closed']))
                                sock_to_rcv.close()
                                sockets_rd.remove(sock_to_rcv)

                                if data[1] in peers_list:
                                    peers_list.remove(data[1])
